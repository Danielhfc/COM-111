...
//Funcoes do Trabalho
//F12
int ArvBinTotalFolhas( ArvBin * raiz){
    if ( raiz == NULL )
    return -1; // Árvore Não Criada
    if (*raiz == NULL) return 0;
    struct NO *atual = *raiz;
    int total,okesq,okdir;
    if((atual->dir==NULL) && (atual->esq==NULL)){
        return 1;
    }
    okesq = ArvBinTotalFolhas(&atual->esq);
    okdir = ArvBinTotalFolhas(&atual->dir);
    return okesq+okdir+total;
}
//F13
int ArvBinNoUmFilho(ArvBin* raiz){
    if ( raiz == NULL ) return -1;
    else if (*raiz == NULL) return -2;
    else{
        if ((((*raiz)->esq == NULL) && ((*raiz)->dir != NULL)) || (((*raiz)->esq != NULL) && ((*raiz)->dir == NULL)))
            ...
            return ArvBinNoUmFilho(&((*raiz )->esq)) + ArvBinNoUmFilho(&((*raiz )->dir));
    }
...
//F14
int ArvBinEstritaBin(ArvBin * raiz ) { 
if (raiz == NULL) return -1; //Arvore Não Criada 
if ( *raiz == NULL) return -2; //Arvore Vazia 
int TotNoEsq = ArvBinEstritaBin ( &((*raiz )->esq)); 
int TotNoDir = ArvBinEstritaBin ( &((*raiz )->dir)); 
if(((TotNoEsq%2)&&(TotNoDir%2))!=0) return 1;
else return 2; 
} 
