...
static Elemento* criar_elemento()
{
...
int inserir_lista_final(Lista *li, Produto *produto)
{
  ...
  if((*li)==NULL){
    ...
    // se sim insere elemento único no início da lista
    no->prox = no;     // próximo elemento na lista circular é ele mesmo
    *li = no;          // 'no' passa a ser o primeiro elemento da lista
    ...
    // se não
           ...
           // cria aux para percorrer a  lista e aponta para o primeiro
      Elemento *aux;
      aux = *li;
    ...
    no->prox = *li;  // 'no' é o próximo elemento na lista circular após o último
    aux->prox = no;  //  primeiro elemento '*li' é o próximo após o 'no'
  }
...
Produto *buscar_lista_posicao(Lista *li, int pos)
{
  ...
  if((*li)==NULL || li==NULL || pos<1)
    return NULL;
  // se sim 
  //posicione na posição1 primeiro elemento
  int i = 1;
  Elemento *no = *li;
...
void imprimir_lista(Lista *li)
{
  ...
  exit (0);
  // se sim
  // posicione no primeiro elemento
  Elemento *no = *li;
